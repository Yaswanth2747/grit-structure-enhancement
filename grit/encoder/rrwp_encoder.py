...
class RRWPLinearNodeEncoder(torch.nn.Module):
    ...
    def __init__(self, emb_dim, out_dim, use_bias=False, batchnorm=False, layernorm=False, pe_name="rrwp"):
        ...
        # Learnable gate for adaptive k
        self.gate_vector = nn.Parameter(torch.ones(emb_dim))
    ...
    def forward(self, batch):
        ...
        # Apply learnable gate (soft mask)
        gate = torch.sigmoid(self.gate_vector)
        rrwp = rrwp * gate
...
class RRWPLinearEdgeEncoder(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        # Apply learnable gate (soft mask) to adaptively scale/mask walk steps
        gate = torch.sigmoid(self.gate_vector)
        rrwp_val = rrwp_val * gate
